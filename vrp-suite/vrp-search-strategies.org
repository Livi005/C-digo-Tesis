...
#+DATE 2018-08-25
#+TODO: HERE TODO WIP BROKEN | DONE
...
***** tests
      ...
      : ==================
      : 
      : Let's add the solution ((1 2 3) (4 5 6)): 0 (expect 0)
      : Let's add the solution ((1 2 3) (4 5 6)): 0 (expect 0)
      : Let's add the solution ((1 2 3) (4 5 6)): 1 (expect 1)
      : Let's add the solution ((1 2 3) (4 5 6)): 1 (expect 1)
...
*** functions to detect repeated elements in a neigborhood using a trie
**** description
    In this section, I'm going to modify the Heidy's idea, by adding a list with all the coordinates that generate the same solution.
**** 2nd trie implementation
***** description
     In this section we write a trie implementation, that will be useful to determine whether or not a solution has already been visited.  In this trie we store in each leaf all the operations that lead to that neighbor.
***** class trie-node
****** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (def-vrp-class trie-node ()
       ((value)
        (children-nodes)
        (parent)
        (vrp-ops))
       :documentation "A node to build a trie to store vrp solutions")

     (defun new-trie-node (value &key children-nodes parent vrp-ops)
...
****** test
   #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp 
   (let* ((t1 (new-trie-node 1 :children-nodes nil :parent nil)))
     (format t "node: ~a~%" t1)
     (format t "value: ~a~%" (value t1))
     (format t "children: ~a~%" (children-nodes t1))
     (format t "parent: ~a~%" (parent t1))
     (format t "vrp-ops: ~a~%" (vrp-ops t1)))
   #+END_SRC

   #+RESULTS:
   : node: #<TRIE-NODE {10093C3F73}>
   : value: 1
   : children: NIL
   : parent: NIL
   : vrp-ops: NIL
...
****** test
   ...
   #+RESULTS:
   : node: #<TRIE-NODE {100A40B7E3}>
   : value: r1
   : children: NIL
   : parent: NIL

***** function insert-in-the-children-list2
****** description
I think that the following code inserts the node =node= in the children-list of node r.
****** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun insert-in-the-children-list2 (r node)
       "Insert the given node in the children list of r."
       (let ((flag 0))
         (loop for child in (children-nodes r)
               while (= flag 0)
               do
               ;; (format t "value of child ~a, value of node ~a~%"
               ;;         (value child)
               ;;         (value node))
               (if (eql (value child) (value node))
                   ;; if values are equal
                   (progn 
                     ...
                     (setf node child))))
         (if (eql flag 0)
             ;; if there is no node with that value, let's add it
             (push node (children-nodes r)))       
         (values node flag)))
      #+END_SRC   
****** test (create a trie)
   #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp 
   (defun make-test-trie ()
     "returns a basic test trie"
      (let* ((t1 (make-trie))
             (n1 (new-trie-node 1 :children-nodes nil :parent t1))
             (n2 (new-trie-node 0 :children-nodes nil :parent t1))
             (n3 (new-trie-node 2 :children-nodes nil :parent t1)))
        ;; let's add 2 nodes to the node t
        (push (new-trie-node 12 :parent n1) (children-nodes n1))
        (push (new-trie-node 13 :parent n1) (children-nodes n1))
        ;; let's add 1 nodes to the node i
        (push (new-trie-node 21 :parent n3) (children-nodes n3))

        ;; let's add the children to the t1
        (push n3 (children-nodes t1))
        (push n2 (children-nodes t1))
        (push n1 (children-nodes t1))
        ;; let's return t1
        t1))

   (defparameter test-trie (make-test-trie))

   (loop for c in (children-nodes test-trie) doing
           (format t "value of node: ~a~%" (value c)))
   #+END_SRC

   #+RESULTS:
   : node: #<TRIE-NODE {10037F7163}>
   : value: r1
   : children: (#<TRIE-NODE {10037F71A3}> #<TRIE-NODE {10037F71E3}>
   :            #<TRIE-NODE {10037F7223}>)
   : parent: NIL

****** test (test the function)
   #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp 
   (let* ((n1 (new-trie-node 1 :parent nil))
          (n2 (new-trie-node 15 :parent nil))
          (tt1 (make-test-trie))
          n f)
     (bformat t "Testing insert-in-the-children-list2")
     (setf (values n f) (insert-in-the-children-list2 tt1 n1))
     (format t "value: ~a, flag ~a~%" n f)
     (setf (values n f) (insert-in-the-children-list2 tt1 n2))
     (format t "value: ~a, flag ~a~%" n f)
  
     (format t "Let's print test-trie~%")
     (loop for c in (children-nodes tt1) doing
           (format t "value of node: ~a~%" (value c))))
   #+END_SRC

   #+RESULTS:
   
***** function insert-vrp-solution-in-trie
****** description
In this section, we implement a function to insert a vrp-solution into the trie.  The vrp-solution should be described as a list of list of clients.
****** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun insert-vrp-solution-in-trie (solution root)
       (let* ((node nil)
              (leaf nil)               
              (visit 0)
              (last-node nil))
         ...
         (loop for route in solution do 
               (loop for client in route do
                     (setf node (new-trie-node client))
                     (setf (parent node) root)
                     (setf root (insert-in-the-children-list2 root node)))
               (setf leaf root)
               (setf root
                     (insert-in-the-children-list2
                      root (new-trie-node "r" :parent leaf))))
...
****** test 1
      ...
      (let* ((sol1 '((1 2 3) (4 5 6)))
             ...
             (format t "Let's add the solution ~a: ~a (expect 0)~%"
                     sol1 (insert-vrp-solution-in-trie sol1 trie))
             (format t "Let's add the solution ~a: ~a (expect 0)~%"
                     sol1 (insert-vrp-solution-in-trie sol2 trie))
             (format t "Let's add the solution ~a: ~a (expect 1)~%"
                     sol1 (insert-vrp-solution-in-trie sol3 trie))
             (format t "Let's add the solution ~a: ~a (expect 1)~%"
                     sol1 (insert-vrp-solution-in-trie sol1 trie)))
      #+end_src
****** test 2 (i want to get the inserted node)
      #+begin_src lisp +n -r :results output :exports code
      (let* ((sol1 '((1 2 3) (4 5 6) (7 8)))
             (sol2 '((1 2 4) (3 5 6)))
             (sol3 '((1 2 3) (4 5 6) (7 8)))
             (trie (make-trie))
             n1 n2 n3 new-or-not
             )

        (bformat t "Testing the node returning trie")

        (setf (values new-or-not n1) (insert-vrp-solution-in-trie sol1 trie))
        (format t "with ~a, returns ~a and node ~a~%"
                sol1 new-or-not n1)
        (push "axb" (vrp-ops n1))

        (setf (values new-or-not n2) (insert-vrp-solution-in-trie sol2 trie))
        (format t "with ~a, returns ~a and node ~a~%"
                sol2 new-or-not n2)
        (push "axc" (vrp-ops n2))

        (setf (values new-or-not n3) (insert-vrp-solution-in-trie sol3 trie))
        (format t "with ~a, returns ~a and node ~a~%"
                sol3 new-or-not n3)
        (push "axd" (vrp-ops n3))

        ;; now I'll traverse the trie
        (labels ((dfs (x current-sol)
                   (push (if (numberp (value x)) (value x) "|") current-sol)
                   (when (> (length (vrp-ops x)) 1)
                     (format t "sol: ~a.~%~{    ~a~%~}~%"
                               (reverse current-sol) (reverse (vrp-ops x))))
                   ;; else
                   (dolist (c (children-nodes x))
                     (dfs c current-sol))))
          (dfs trie nil)))
      #+end_src
***** function check-if-vrp-solution-is-in-trie
****** description
In this section, we implement a function to check whether or not the vrp-solution exists already in the trie.  The vrp-solution should be described as a list of clients with the routes separated by R.
****** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun check-if-vrp-solution-is-in-trie (solution trie)
       (let* ((solution-found nil))
         (labels ((dfs (sol node)
                      (let* ((current-client (first sol))
                             (next-clients (rest sol)))
                        (if (null current-client)
                            ;; return t
                            (progn
                              (setf solution-found t))
                            ;; else
                            ;; let's see if it is one of the children
                            (loop while (not solution-found)
                                  for child in (children-nodes node)
                                  do
                                  (if (obj= current-client (value child))
                                      (dfs next-clients child))
                                  finally (setf solution-found nil))))
...
****** test 1
      ...
      (let* ((sol1 '((1 2 3) (4 5 6)))
             ...
             (format t "check: ~a~%"
                     (check-if-vrp-solution-is-in-trie `(1 2 3 "R" 4 5 6 )
                                                       trie))
             )
      #+end_src
****** test 2 (i want to get the inserted node)
      ...
      (let* ((sol1 '((1 2 3) (4 5 6) (7 8)))
        ...
        (labels ((dfs (x current-sol)
          ...
          (dfs trie nil)))
      #+end_src
**** function to print all the solutions in the vrp-trie
***** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun print-all-entries-in-vrp-trie (trie &key (stream t))
       (labels ((dfs (x current-sol)
                  (if (and (not (parent x))
                           (null (children-nodes x)))
                      (format stream "  empty trie~%")
                   (if (parent x)
                      (push (if (numberp (value x))
                            (value x)
                            ;; else
                            "|")
                        current-sol)))
                  (if (> (length (vrp-ops x)) 0)
                      (format stream "sol: ~a.~%~{    ~a~%~}~%"
                              (reverse current-sol) (reverse (vrp-ops x))))
                  ;; else
                  (dolist (c (children-nodes x))
                    (dfs c current-sol))))
...
***** test
   ...
   (let* ((sol1 '((1 2 3) (4 5 6) (7 8)))
        ...
        ;; now I'll print the trie
        (print-all-entries-in-vrp-trie trie :stream t)
        )
   #+END_SRC
   ...
   #+begin_example
   =================================
    Testing the node returning trie
   =================================

   with ((1 2 3) (4 5 6) (7 8)), returns 0 and node #<TRIE-NODE {100351B103}>
   with ((1 2 4) (3 5 6)), returns 0 and node #<TRIE-NODE {100351C1C3}>
   with ((1 2 3) (4 5 6) (7 8)), returns 1 and node #<TRIE-NODE {100351B103}>
   sol: (| 1 2 3 | 4 5 6 | 7 8).
       axb
       axd

   #+end_example
**** function to count all the coordinates for all solutions in the vrp-trie
***** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun get-all-coords-for-all-solutions-in-vrp-trie (trie &key (stream t)
                                                                   (print-coords nil)
                                                                   (print-sols nil))
       (let* ((result nil))
         (labels ((dfs (x current-sol)
                    ;; let's build the solution up to this point
                    ;; but only if we are not at the root node
                    (if (parent x)
                        (push (if (numberp (value x))
                                  (value x)
                                  ;; else
                                  "|")
                              current-sol))
                    ;; if we have the slot vrp-ops
                    ;; then we are in a leaf
                    (if (> (length (vrp-ops x)) 0)
                        (progn
                          ;; let's push the list with solution
                          ;; and number of coords into result
                          (push (list (reverse current-sol)
                                      (length (vrp-ops x)))
                                result)
                          (if print-sols
                              (format stream "sol: ~a: ~a~%"
...
****** test
   ...
   (let* ((sol (make-basic-solution-from-list 1 '((1 2 3) (4 5))))
          (wc (basic-working-copy sol))
          (ops-list nil)
          ;; first we initialize the trie
          (neighborhood-trie (make-trie))
          ;; the trie for the whole neighborhood
          (original-neighborhood-trie (make-trie))
          ;; the following is required for the insertions
          (node-in-the-trie-where-the-solution-was-inserted nil)
          (insertion-result nil)
          ;; this is to count the number of solutions
          ;; using the trie
          (number-of-solutions-in-original nil)
          (ways-to-obtain-the-solution 0)
          (solution nil))

     (declare (ignorable ways-to-obtain-the-solution solution))

     (bformat t "Testing get-all-coordinates-for-solution-in-vrp-trie")
       ;; let's prepare the solution for the exploration
       ;; otherwise, the macros won't work
       (prepare-solution-for-neighborhood-exploration wc)

       ;; let's add the initial solution to the tries
       (insert-vrp-solution-in-trie (ops-list-to-list-of-clients nil wc)
                                    neighborhood-trie)

       (insert-vrp-solution-in-trie (ops-list-to-list-of-clients nil wc)
                                    original-neighborhood-trie)
       ;; let's write the original-criterion
       ;; let's write the criterion
       (doselect-route (r1 wc)
         (doselect-route (r2 wc)
           (doselect-client (c1 r1 wc)
             (doselect-client (c2 r2 wc :ex-cond
                                  (or
                                   (< r2 r1)
                                   (and
                                    (= r1 r2)
               ...
               (doinsert-client (c1 r2 wc)
                 (doinsert-client (c2 r1 wc)

                   ;; let's insert the current solution in the trie
                   (setf (values insertion-result
                                 node-in-the-trie-where-the-solution-was-inserted)

                          (insert-vrp-solution-in-trie
                           (ops-list-to-list-of-clients ops-list wc)
                           original-neighborhood-trie))

                   ;; let's store the operations in the inserted node
                   ;; if there is more than one way to obtain this solution
                   ;; it will have more than one list-of-operations

                   (push (reverse (clone ops-list))
                         (vrp-ops node-in-the-trie-where-the-solution-was-inserted))))))))

       ;; now in the original-neighborhood-trie we have all the solutions
       ;; let's print that info

       (setf number-of-solutions-in-original
             (count-repeated-solutions-in-vrp-trie
              original-neighborhood-trie))

       (format t "~{  ~a ~%~}"
               (sort (get-all-coords-for-all-solutions-in-vrp-trie
                      original-neighborhood-trie)
                     (lambda (x y) (<= (second x) (second y)))))

       ;; (print-all-coordinates-for-solution-in-vrp-trie
       ;;          (convert-vrp-solution-to-trie-list '((1 2 3 4)))
       ;;          original-neighborhood-trie)

       ;; (setf solution '((2 1 4 3)))

       ;; (setf ways-to-obtain-the-solution
       ;;       (get-all-coordinates-for-solution-in-vrp-trie
       ;;        (convert-vrp-solution-to-trie-list solution)
       ;;        original-neighborhood-trie))

       ;; (format t "~%solution: ~a~%coordinates: ~a~%~{  ~a~%~}"
       ;;         solution
       ;;         (length ways-to-obtain-the-solution)
       ;;         ways-to-obtain-the-solution)


       )
   #+END_SRC

**** function to print the repeated entries in the vrp-trie
***** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun print-repeated-entries-in-vrp-trie (trie &key (stream t))
       (labels ((dfs (x current-sol)
                  (if (parent x)
                      (push (if (numberp (value x))
                            (value x)
...
***** test
   ...
   (let* ((sol1 '((1 2 3) (4 5 6) (7 8)))
        ...
        ;; now I'll print the trie
        (print-repeated-entries-in-vrp-trie trie :stream nil)
        )
   #+END_SRC
...
**** function to count the number of repeated solutions
***** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun count-repeated-solutions-in-vrp-trie (trie)
       (let* ((unique-solutions 0)
              (repeated-solutions 0))
         (labels ((dfs (x)
                  (when (vrp-ops x)
                    (incf unique-solutions)
                    (incf repeated-solutions))
                  ;; if there is more than 1 way to obtain the solution
                  ;; then we must say that all but one are repeated
                  (if (> (length (vrp-ops x)) 1)
                      (incf repeated-solutions (1- (length (vrp-ops x)))))
                  ;; if there are not vrp-ops in this node
                  ;; we are not in a leave, so we must keep goingh
                  (unless (vrp-ops x)
                        (dolist (c (children-nodes x))
                    (dfs c)))))
           (dfs trie))
         ;; return a list with the values
         (list unique-solutions repeated-solutions (- repeated-solutions unique-solutions))))
     #+END_SRC   
***** simple test
   #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp 
   (let* ((sol1 '((1 2 3) (4 5 6) (7 8)))
        ...
        ;; now I'll print the trie
        (print (count-repeated-solutions-in-vrp-trie trie))
        )
   #+END_SRC
...
**** function to convert a vrp-solution to a list of clients
***** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun convert-vrp-solution-to-trie-list (sol)
       "Sol is a solution described as a list of lists. A trie-list is a one dimensional list where the end of route is represented by a the string r"
       (let* ((result nil))
         (loop for route in sol
               do
               (unless (null result) (push "r" result))
               (loop for client in route
                     do
                     (push client result)))
         ;; let's return the rest reverse of result
         (reverse result)))
     #+END_SRC   
***** test
   #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp 
   (let* ((sol1 '((1 2 3) (4 5 6) (7 8)))
        ...
        (bformat t "Testing convert-vrp-solution-to-trie-list")

        (format t "sol1: ~a~%" (convert-vrp-solution-to-trie-list sol1))
        (format t "sol2: ~a~%" (convert-vrp-solution-to-trie-list sol2))
        (format t "sol3: ~a~%" (convert-vrp-solution-to-trie-list sol3)))
   #+END_SRC

   #+RESULTS:
   : ===========================================
   :  Testing convert-vrp-solution-to-trie-list
   : ===========================================
   : 
   : sol1: (1 2 3 r 4 5 6 r 7 8)
   : sol2: (1 2 4 r 3 5 6)
   : sol3: (1 2 3 4)

**** function to get all the ways to obtain a solution
***** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun get-all-coordinates-for-solution-in-vrp-trie (sol trie &key (stream t))
       "Sol is a solution described as a list where the routes are separated by the string r."
       (declare (ignorable stream))
       ;; if solution is nil
       ;; then we found the node we are looking for,
       ;; so let's return all the ways to get that solution
       (if (null sol)
           ;; return the slot vrp-ops
           (vrp-ops trie)
           ;; else
           ;; let's search in the children of trie
           (let* ((current-client (first sol)))
             (loop for node in (children-nodes trie)
                   doing
                     (if (eql current-client (value node))
                             (return
                               (get-all-coordinates-for-solution-in-vrp-trie
                                (rest sol) node)))
                   ;; if we reach this point the solution is not in the trie
                   finally (format t "solution not in trie")))))
     #+END_SRC   
***** test
   #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp 

   (let* ((sol (make-basic-solution-from-list 1 '((1 2 3 4))))
          (wc (basic-working-copy sol))
          (ops-list nil)
          ;; first we initialize the trie
          (neighborhood-trie (make-trie))
   ...
   #+end_example
**** function to compute the difference between two vrp tries
This function receives two tries and returns a new trie with the difference
***** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defun vrp-trie-difference (trie1 trie2)
       (let* ((result (make-trie))
              (sol-exists-in-second-trie nil)
              (node-in-trie2 nil))

         (declare (ignorable result
                             sol-exists-in-second-trie
                             node-in-trie2))

         (labels ((dfs (x current-sol current-route)
                    ;; let's build the solution as a list of list
                  (if (parent x)
                      (if (numberp (value x))
                            (push (value x) current-route)
                            ;; else
                            (progn
                              (push (reverse current-route)
                                    current-sol)
                              (setf current-route nil))))
                  ;; here we have in current-sol the solution as list
                  (if (> (length (vrp-ops x)) 0)
                      ;; if the is non nil
                      ;; then we are in a final node
                      (progn
                        ;; first of all, let's add the final route
                        (push (reverse current-route) current-sol)

                        ;; here we look for that solution
                        ;; in the second trie
                        (setf (values sol-exists-in-second-trie
                                      node-in-trie2)
                              (insert-vrp-solution-in-trie
                               (reverse current-sol)
                               trie2))

                        ;; (format t "debug: analyzing sol: ~a. ~a~%"
                        ;;         (reverse current-sol)
                        ;;         sol-exists-in-second-trie)

                        ;; (format t "debug:~%sol-in-trie2: ~a~%"
                        ;;         sol-exists-in-second-trie)
                        ;; if the solution does not exists in the second trie
                        ;; then it is in the diffence
                        (unless (= sol-exists-in-second-trie 1)

                          ;; (format t "inserting in result solution ~a~%"
                          ;;         current-sol)
                          ...
                          (setf (values sol-exists-in-second-trie
                                ...
                                (insert-vrp-solution-in-trie
                                 (reverse current-sol)
                                 result))
                          ;; now let's insert the vrp-ops
                          ;; in the inserted node
                          (setf (vrp-ops node-in-trie2)
                                (clone (vrp-ops x))))
                        ;; (format t "~a~%" (reverse current-sol))
                        ))
                  ;; else
                  (dolist (c (children-nodes x))
                    (dfs c current-sol current-route))))
           (dfs trie1 nil nil)
...
***** test
   ...
   (let* ((sol1 '((1 2 3) (4 5 6) (7 8)))
        ...
        ;; let's insert sol 2 it into trie2
        (setf (values new-or-not n1) (insert-vrp-solution-in-trie sol2 trie2))
        (push "axb" (vrp-ops n1))
     

        (format t "trie1:~%")
        (print-all-entries-in-vrp-trie trie )

        (format t "trie2:~%")
        (print-all-entries-in-vrp-trie trie2 )
     
     
        ;; now I'll print the trie
        (format t "computing the difference~%")
        (setf difference-trie (vrp-trie-difference trie trie2))

        (format t "printing all solutions in difference-trie~%")
        (print-all-entries-in-vrp-trie difference-trie )
        )
   #+END_SRC
   ...
   #+end_example
*** classes for the strategy
**** class definitions for the strategy
***** find-repeated-solutions-and-operations-strategy (functional)
****** description
     This class represents a search where we find the repeated neighbors in the neighborhood, and for each neighbor we store in a trie the ops required to obtain them.  When the length of this list of vrp-ops is greater than 1, then it is a repeated element.
****** code
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (def-vrp-class find-repeated-solutions-and-operations-strategy
      (search-with-a-basic-wc)
       ()
      :documentation "A class to represent a search where we store the vrp-ops required to build each neighbor."
      :constructor (find-repeated-solutions-and-operations-strategy ())
      :print-object-string ("<find-repeated-solutions-and-operations-strategy>")
      :slots-for-obj= ()
      :slots-for-clone ())
     #+END_SRC
...
****** test
     #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp
     (let* ((st1 (find-repeated-solutions-and-operations-strategy)))
       (bformat t "Testing find-repeated-solutions-and-operations-strategy...")
       (format t "find-repeated-solutions-and-operations-strategy ~a~%" st1))
     #+END_SRC

****** constant
     Here we create a constant (variable?) with an instance of this class.
     #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
     (defparameter +find-repeated-solutions-and-operations-strategy+
       (find-repeated-solutions-and-operations-strategy))
     #+END_SRC
**** generate-inside-let-initializations
***** description
    When using the find-repeated-solutions-and-operations-strategy, we need to initialize the trieE, and the current solution.  We also need to initialize the node where the solution is created and the return value of the insertion in the trie.
***** method :after(t t find-repeated-solutions-and-operations-strategy t)
****** code
      #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
      (defmethod generate-inside-let-initializations :after
          (description
           strategy
           (other-strategies find-repeated-solutions-and-operations-strategy)
           (where-to-store-it basic-neighborhood-strategy-blueprint))

        (push `(current-trie (make-trie)) ;; the trie to store the visited clients
              (initializations-inside-the-let where-to-store-it))
        (push `(current-node nil) ;; the node in the trie where the solution is stored
              (initializations-inside-the-let where-to-store-it))
        (push `(insertion-result nil) ;; the result of the insertion
              (initializations-inside-the-let where-to-store-it)))
      #+END_SRC
****** test
     #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp
     ...
     #+RESULTS:
     : ================================================
     :  Testing generate-inside-let-initializations...
     : ================================================
     : 
     : let-initializations: ((CURRENT-TRIE (MAKE-TRIE)) (SOLUTIONS-REPEATED NIL)
     :                       (WC (MAKE-WORKING-COPY (CLONE SOLUTION))) (OPS-LIST NIL)
     :                       (*VRP-STOP-NEIGHBORHOOD-SEARCH* NIL))
...
****** code
      ...
      (defmethod generate-outside-let-initializations :after
          ...
          (push `(insert-vrp-solution-in-trie (ops-list-to-list-of-clients nil wc)
                                  current-trie)
           (initializations-outside-the-let where-to-store-it)))
      #+END_SRC
****** test
     ...
     : =================================================
     : 
     : let-initializations: ((INSERT-VRP-SOLUTION-IN-TRIE
     :                        (OPS-LIST-TO-LIST-OF-CLIENTS NIL WC) CURRENT-TRIE)
     :                       (PREPARE-SOLUTION-FOR-NEIGHBORHOOD-EXPLORATION WC))
**** generate-code-inside-macros
***** description
    Inside the macros we need to decide what to do. In this case is to add it to the trie and then store the ops that generated the current solution.
***** method :after(t t find-repeated-solutions-and-operations-strategy t)
****** code
      #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
      (defmethod generate-code-inside-macros :after
        ...
        (push `(progn
                 ;; first let's insert the current solution in the trie
                 (setf (values insertion-result current-node)
                       (insert-vrp-solution-in-trie
                        (ops-list-to-list-of-clients ops-list wc)
                        current-trie))
                 ;; now let's store the operations in the node
                 (push (reverse (clone ops-list)) (vrp-ops current-node))

                 ;; if we want, we could use the the value of
                 ;; insertion-result to compute something
                 )
...
**** generate-return-code
***** description
    In this case, we just return the trie
***** method :after(t t repeated-solutions-search-strategy t)
****** code
      #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
      (defmethod generate-return-code :after
...
***** testing the generated code
     ...
     (let* ((*vrp-unit-testing-display-output* nil)
       ...
       (prepare-blueprint-for-code-generation
        ...
        ;; (random-neighborhood-search-strategy 100)
        +exhaustive-search-strategy+
        (make-instance 'find-repeated-solutions-and-operations-strategy)
        bp1)

       (format t "the generated code:~%~a~%"
               (write-neighborhood-exploration-code bp1)))
     #+END_SRC
***** testing the printing of the repeated solutions
     #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp
     (with-basic-solution (s1 ((1 2 3) (4 5)))
       (let* ((bp1 (basic-strategy-blueprint))
              (wc (basic-working-copy s1))
         ...
         (setf repeated-elements (funcall
                                  explore-function
                                  s1
                                  t
                                  t))

         (format t "repeated-elements: ~a~%" repeated-elements)

         (format t "Now let's check the repeated solutions~%")

         (print-repeated-entries-in-vrp-trie repeated-elements)

         ))
     #+END_SRC
***** counting the number of repeated solutions
     #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp
     (with-basic-solution (s1 ((1 2 3 4 5)))
       (let* ((bp1 (basic-strategy-blueprint))
              (wc (basic-working-copy s1))
         ...
         (format t "let's print all the solutions in the trie~%")

         ;; (print-all-entries-in-vrp-trie repeated-elements)
         ...
         (print (count-repeated-solutions-in-vrp-trie repeated-elements))

         ))
     #+END_SRC
*** function to count and analyze neighborhoods
**** description
In this section I'll create a function that receives a solution an a criterion and analyzes the neighborhood of that solution.  The criterion should be defined as the list of instructions.

 - It has 4 keyword parameters:
   - show-numbers, defaults to t.  It prints 3 numbers:
     - the number of unique solutions
     - the number of coordinates
     - the number of "repeated solutions"
   - show-all, defaults to nil
     When non nil prints (to the given stream) all the solutions with its operations.
   - show-repeated, defaults to nil
     When non nil shows the repeated elements with all the operations that produce the same solution.
   - stream, defaults to t
     Is the stream to where all the "things" should be printed.

**** code
    #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
    (defun analyze-neighborhood (solution criterion &key
                                                      (show-numbers t)
                                                      (show-all nil)
                                                      (show-repeated nil)
                                                      (stream t))
      "Function that receives a solution an a criterion and analyzes the neighborhood of that solution.  The criterion should be defined as the list of instructions."

      (let* ((sol (make-basic-solution-from-list 1 solution))
             ;; (wc (basic-working-copy sol))
             (exploration-function (make-neighborhood-criterion
                                    criterion
                                    +exhaustive-search-strategy+
                                    +find-repeated-solutions-and-operations-strategy+))
         
             (neighborhood-trie (funcall exploration-function sol t t))
             (number-of-solutions
              (count-repeated-solutions-in-vrp-trie neighborhood-trie)))

        (if show-all
            (print-all-entries-in-vrp-trie neighborhood-trie :stream stream))

        (if show-repeated
            (print-repeated-entries-in-vrp-trie neighborhood-trie :stream stream))

        (if show-numbers
            (format stream "~a~%" number-of-solutions))
    
        ;; let's count the the repeated-elements
        number-of-solutions))
    #+END_SRC
...
**** test
     #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp
     (progn
       (bformat t "Testing analyze-neighborhood")

       (print (analyze-neighborhood '((1 2 3 4 5))
                                    '((select-route r1)
                                      (select-client c1 from r1)
                                      (select-route r2)
                                      (select-client c2 from r2)
...
**** examples for Hermen
     #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp
     (progn
       (bformat t "Examples for Hermen")

       (let* (;; the solutions
              (s1 '((1 2 3 4 5)))
              (s2 '((1 2 3) (4 5)))
              (s3 '((1 2 3 4 5)))
              (list-of-solutions (list s1 s2 s3))
              ;; the criteria
              (c1 rab)
              (c2 rarb)
              (c3 rarac)
              ...
              (c6 '((select-route r1)
                    ...
                    (insert-client c1 into r1)
                    (insert-client c2 into r1)))
              (swap-in-route '((select-route r1)
                               (select-client c1 from r1)
                               (select-client c2 from r1)
                               (swap-clients c1 c2)))
         
              (list-of-criteria (list c1 c2 c3 c4 c5 c6)))
         (declare (ignorable list-of-criteria list-of-solutions))

         ;; (loop for s in list-of-solutions
         ;;       do
         ;;       (loop for c in list-of-criteria
         ;;             do
         ;;             (format t "==============================~%")
         ;;             (format t "Analying solution ~a~%with criterion:~%~{~10t~a~%~}%"
         ;;                     s c)
         ;;             (format t "==============================~%")
         ;;             (analyze-neighborhood s c :show-repeated t :show-numbers t)))


         (with-open-file (f "/tmp/hermen1.txt"
                            :direction :output
                            :if-exists :supersede)
         (format f "==============================~%")
         (format f "Analyzing solution ~a~%with criterion:~%~{~10t~a~%~}"
                 s1 c6)
         (format f "==============================~2%")
            
         (analyze-neighborhood s1 c6
                               :show-repeated t
                               :show-numbers t
                               :stream f))))
...
*** Analysis of neighborhood rarab21b12
**** description
In this section I'll create a function that receives a solution and analyzes the neighborhood of that solution.  I'm going to use the macros in order to use the constraints.  
...
In the 2nd version we also have a trie with all the solutions in the neighborhood.  This is to know for sure how many actual solutions are there.

**** code v2
    #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
    (defun analyze-rarab21b12 (solution &key
                                          (show-numbers nil)
                                          (show-numbers-in-original nil)
                                          (show-all nil)
                                          (show-repeated nil)
                                          (show-difference nil)
                                          (stream t))
      ...
      (let* ((sol (make-basic-solution-from-list 1 solution))
             ...
             ;; this is to count the number of solutions
             ;; using the trie
             (number-of-solutions nil)
             (number-of-solutions-in-original nil)

             (ways-to-obtain-the-solution nil)

             (difference-trie nil)
             )

        (declare (ignorable neighborhood-trie
                            number-of-solutions
                            ways-to-obtain-the-solution))
        ...
        ;; let's add the initial solution to the original trie
        (insert-vrp-solution-in-trie (ops-list-to-list-of-clients nil wc)
                                     original-neighborhood-trie)
        ;; but not to the smart one
        ...
        (doselect-route (r1 wc)
          (doselect-route (r2 wc)
            (doselect-client (c1 r1 wc)
              (doselect-client (c2 r2 wc :ex-cond (or (< r2 r1)
                                                      (and (= r2 r1)
                                                           (< c2.position
                                                              c1.position))))
                (doinsert-client (c1 r2 wc)
        ...
        ;; let's write the modified criterion
        (doselect-route (r1 wc)
          (doselect-route (r2 wc)
            (doselect-client (c1 r1 wc)
              (doselect-client (c2 r2 wc :ex-cond (or (< r2 r1)
                ...
                (doinsert-client (c1 r2 wc  :ex-cond
                                     (or
                                      ;; let's forbid the insertion
                                      ;; just before the extraction pos
                                      ;; As c2 is always after c1,
                                      ;; the selection pos of c2 does not
                                      ;; affect the condition for c1
                                      (and
                                       (= r2 c1.route)
                                       (= c1.ins.pos (1- c1.position)))
                                      ;; the only pos that can be inserted in
                                      ;; the same position is the 1
                                      (and
                                       (= r2 c1.route)
                                       (= c1.ins.pos c1.position)
                                       (not (= c1.position 1)))))

                  (doinsert-client (c2 r1 wc  :ex-cond
                                       (or
                                        ;; let's forbid the insertion
                                        ;; just before the extraction pos.
                                        ;; It may depend on the relative
                                        ;; positions of the c1 and c2
                                        ;; insertions
                                        ;; FIX FIX FIX
                                        ;; FIX FIX FIX
                                        ;; right now 2023-08-01 we are computing
                                        ;; wrong the insert position :-(.
                                        ;; We are forbidding allowable positions
                                        ;; 
                                        (and
                                         ;; both insertion are in the same route
                                         (= r1 c2.route)
                                         ;; allow the insertion of c2 in pos 1
                                         ;; if it was selected from pos 1
                                         ;; I think that this 'not' should
                                         ;; break the 'and'
                                         ;; SO FAR (2023-07-28) IT WORKS 
                                         (not (and (= c1.position 1)
                                                   (>= c1.ins.pos 1)
                                                   (= c2.position 1)
                                                   (= c2.ins.pos 1)))
                                         ;; forbid the insertion
                                         ;; just before the extraction point
                                         ;; FIX FIX FIX FIX FIX   
                                         ;; FIX FIX FIX FIX FIX   
                                         (if (and
                                              ;; the insertion route of
                                              ;; c1 is also in r1
                                              (= r1 r2)
                                              ;; c1 was inserted before c2
                                              (<= c1.ins.pos c2.ins.pos))
                                             ;; then the previous position
                                             ;; is the same selection position
                                             (= c2.ins.pos c2.position)
                                             ;; else (c1 was inserted after
                                             ;; c2)
                                             (= c2.ins.pos (1- c2.position))))
                                        ;;
                                        ;;
                                        ;; let's forbid the insertion
                                        ;; of c2 in the same position
                                        ;; from where it was selected.
                                        ;; there are two ways to do this:
                                        ;; inserting in 1+ (if c1 is
                                        ;; inserted before c2) and inserting
                                        ;; in the same position (if c1 is
                                        ;; inserted after c2)
                                        ;; 
                                        ;; ;; there is a situation where we should
                                        ;; ;; allow the insertion of c2 in the same
                                        ;; ;; pos from where it was extracted.
                                        ;; ;; It is the only way to move the
                                        ;; ;; client in the first position.
                                        ;; ;;
                                        ;; ;; To do that, we need to forbid
                                        ;; ;; inserting c2 in the same position
                                        ;; ;; except if c1 was selected from pos1
                                        ;; ;; and c2 was selected from pos 1.
                                        (or
                                         (and
                                          (= r1 r2)
                                          ;; allow the insertion of c2 in pos 1
                                          ...
                                          (not (and (= c1.position 1)
                                                    ...
                                                    (= c2.position 1)
                                                    (= c2.ins.pos 1)))
                                          ;; here is where we forbid
                                          ;; inserting in 1-
                                          (if (< c1.ins.pos c2.ins.pos)
                                              ;; c1 was inserted before c2
                                              ;; let's forbid 1+ e. c2.position
                                              (= c2.ins.pos (1+ c2.position))
                                              ;; else
                                              ;; forbid the position without the 1+
                                              (= c2.ins.pos c2.position))))

                                        ;; let's not insert both clients
                                        ;; in the same pos
                                        ;; unless they are both 1 and 1
                                        (and
                                         (= r2 c1.route)
                                         (= c1.ins.pos c1.position)
                                         (= r1 c2.route)
                                         ...
                                         (not (and (= c1.position 1)
                                                   (= c2.position 1)
                                                   (= c2.ins.pos 1)))
                                         ;; we need to add 1+ to c2.position
                                         ;; because when we insert c1
                                         ;; the selected position of c2 is
                                         ;; incremented, and if c1 is inserted
                                         ;; in the same place from where it
                                         ;; was selected, it is before c2.position
                                         (= c2.ins.pos (1+ c2.position)))

                                        ;; ;; let's forbid the insertion of
                                        ;; ;; c1 and c2 in two consecutive
                                        ...
                                        ;; ;; here here here
                                        (and
                                         ;; both clients are inserted in
                                         ;; the same route
                                         (= c1.route r2)
                                         (= c2.route r2)
                                         ;; the insertions are consecutive
                                         (=  c2.ins.pos (1+ c1.ins.pos))
                                         (>  c1.ins.pos c1.position)
                                         )
        ...
        (setf number-of-solutions
              (count-repeated-solutions-in-vrp-trie neighborhood-trie))

        (setf difference-trie (vrp-trie-difference
                               original-neighborhood-trie
                               neighborhood-trie))
        ...
        ;;   ;; (format t "~%original~% solution: ~a~%coordinates: ~a~%~{  ~a~%~}"
        ;;   ;;         solution
        ;;   ;;         (length ways-to-obtain-the-solution)
        ;;   ;;         ways-to-obtain-the-solution)
        ...
        ;;   ;; (format t "~%smart~% solution: ~a~%coordinates: ~a~%~{  ~a~%~}"
        ;;   ;;         solution
        ;;   ;;         (length ways-to-obtain-the-solution)
        ;;   ;;         ways-to-obtain-the-solution)
        ;;   )

        (if show-numbers-in-original
            (format stream "original: ~a~%" number-of-solutions-in-original))

        (if show-numbers
            (format stream "smart: ~a~%" number-of-solutions))


        (if show-difference
            (progn
              (format stream "Missing neighbors:~%")
              (print-all-entries-in-vrp-trie difference-trie)))



        ;; (print-all-entries-in-vrp-trie neighborhood-trie)



        ;; let's count the the repeated-elements
        (list number-of-solutions
              original-neighborhood-trie
              neighborhood-trie)))
    #+END_SRC
...
**** code v1
    ...
    (defun analyze-rarab21b12% (solution &key
      ...
      (let* ((sol (make-basic-solution-from-list 1 solution))
        ...
        (declare (ignorable neighborhood-trie number-of-solutions))
        ...
        ;; let's add to the trie the initial solution 
                ...
                (doinsert-client (c1 r2 wc  :ex-cond
                  ...
                  (doinsert-client (c2 r1 wc  :ex-cond
                                       (or
                                        ...
                                        ;; ;; let's forbid the movement of subroutes
                                        ...
                                        ;;  (= c1.route r2)
                                        ;;  (= c2.route r2)
                                        ;;  ;; c2 is right behind c1
                                        ;;  (= c1.position c2.position)
                                        ;;  ;; the insertions are together
                                        ;;  (=  c2.ins.pos (1+ c1.ins.pos))
                                        ;;  ;; and are before the selecions
                                        ;;  (<= c1.ins.pos c1.position)
                                        ;;  )
...
***** test 1
     ...
     (let* ((numbers nil)
       ...
       (defparameter trie1 nil)
       (defparameter trie2 nil)

       (progn
         (bformat t "Testing analyze-rarab21b12")

         (setf results (analyze-rarab21b12 '((1 2 3 4 ))
                                           :show-numbers-in-original t
                                           :show-numbers t
                                           :show-repeated t
                                           :show-difference t))
         (setf numbers (first results)
               trie1 (second results)
               trie2 (third results))
     ...
     #+END_SRC
*** HERE macro to create functions to analize conditions in the coords
**** description
The idea of the functions in this section is to receive a ops-list and return the constraints (or prohibitions) that it satisfies.

The idea is to create a macro to abstract a lot of boilerplate and to be able to generate both things at the same time: a function to test for a constraint and a symbol-macrolet to insert in the description of the neighborhood.

So far (as of 2024-06-19) I don't know how to do the symbol-macrolet part :-(, so I'll just start with the function to identify the repeated elements.

In this section I'll write the macro for the function for a specific neighborhood (rarab21b12),  the one we are analyzing right now.
**** macro definition
***** code
    #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
    (defmacro make-rarab21b12-constraint (name &body body)
      "In this macro we bind the same variables that appear in the constraints."

      `(defun ,name (ops-list)
        "ops-list is the operations that we need to check whether or not it violates some of the constraints. In this case, as it is just for the neighborhood rarab21b12, the ops is always a list with (a a b b), and the coordinates are in the classes a and b."
        ;; in the let create the names r1, r2, c1.position, etc...
        (let* ((a1 (first ops-list))
               (a2 (second ops-list))
               (b1 (third ops-list))
               (b2 (fourth ops-list))
               (r1 (route a1))
               ...
               (c1.ins.pos (pos b1))
               (c2.ins.pos (pos b2)))
          (declare (ignorable r1 r2
                              c1.route c2.route
                              c1.position c2.position
                              c1.ins.route c2.ins.route
                              c1.ins.pos c2.ins.pos))
          ,@body
          )))
    #+END_SRC
...
***** test
     #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp
     (let* ((ops-list (list (op-select-client 1 2 0)
                            (op-select-client 2 3 1)
                            (op-insert-client 2 4 0)
                            (op-insert-client 1 1 1))))

       (make-rarab21b12-constraint just-print
         (format t "ops-list: ~a:~% r1: ~a~% r2: ~a~% c1.route: ~a~% c2.route: ~a~% c1.position: ~a~% c2.position: ~a~% c1.ins.pos: ~a~% c2.ins.pos: ~a~%"
                 ops-list
                 r1
                 r2
                 c1.route
...
**** HERE write the actual constraints
 In this section I'll add the constraints.  
 The first one will be to detect if the second selection is before the first one

***** second-selection-before-the-first-one
In this constraint we detect whether the second selection is made "before" the first one.
****** code
    #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/vrp-search-strategies.lisp 
    (make-rarab21b12-constraint 2nd-selection-before-1rs-one
      (if (or (< c2.route c1.route)
              (and (= c2.route c1.route)
                   (< c2.position c1.position)))
          '2nd-selection-before-1st-one))
    #+END_SRC

****** tests
     #+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/vrp-tests.lisp
     (let* ((ops-list1 (list (op-select-client 2 3 0)
                             (op-select-client 1 2 1)
                             (op-insert-client 2 4 0)
                             (op-insert-client 1 1 1)))
            (ops-list2 (list (op-select-client 2 3 0)
                             (op-select-client 2 1 1)
                             (op-insert-client 2 4 0)
                             (op-insert-client 1 1 1)))
            (ops-list3 (list (op-select-client 2 3 0)
                             (op-select-client 2 3 1)
                             (op-insert-client 2 4 0)
                             (op-insert-client 1 1 1)))
            (ops-list4 (list (op-select-client 2 3 0)
                             (op-select-client 2 4 1)
                             (op-insert-client 2 4 0)
                             (op-insert-client 1 1 1)))
            (ops-list5 (list (op-select-client 2 3 0)
                             (op-select-client 3 4 1)
                             (op-insert-client 2 4 0)
                             (op-insert-client 1 1 1))))


       (bformat t "Testing 2nd-selection-before-1st-one")
       (format t "(Expect non-nil)~%   in ~a:~%     ~a~%"
               ops-list1
               (2nd-selection-before-1rs-one ops-list1))
       (format t "(Expect non-nil)~%   in ~a:~%     ~a~%"
               ops-list2
               (2nd-selection-before-1rs-one ops-list2))
       (format t "(Expect nil)~%   in ~a:~%     ~a~%"
               ops-list3
               (2nd-selection-before-1rs-one ops-list3))
       (format t "(Expect nil)~%    in ~a:~%     ~a~%"
               ops-list4
               (2nd-selection-before-1rs-one ops-list4))
       (format t "(Expect nil)~%    in ~a:~%     ~a~%"
               ops-list5
               (2nd-selection-before-1rs-one ops-list5))
